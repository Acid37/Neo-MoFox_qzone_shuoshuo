...
- **推荐入口命令**：`/send_feed`、`/read_feed`、`/auto_monitor`
- **生产状态**：✅ 已就绪，可直接用于生产环境
...
### 2) AI 驱动的智能互动
- **AI 评论生成**：基于 LLM + 可选 VLM 图片理解，生成自然、贴合语境的评论
...
- 跳过本人动态的自动互动
- 返回简洁回执，不展示逐条正文
- 如需允许本人动态互动：加 `--allow-self-interact`
- 如需展示逐条列表：加 `--show-list`
...
**示例**：
```
/auto_monitor start              # 启动监控（使用默认配置）
/auto_monitor start --interval 600  # 启动，间隔 10 分钟
/auto_monitor stop               # 停止监控
/auto_monitor status             # 查看监控状态
```
...
| `comment_shuoshuo` | 评论/回复 | `shuoshuo_id`(必填), `content`(必填), `owner_qq`(可选), `comment_id`(可选) |
| `read_shuoshuo` | 阅读说说 | `count`(可选), `offset`(可选), `qq_number`(可选) |
| `auto_monitor` | 自动监控控制 | `action_type=start/stop/status`(必填) |
...
| `interval` | int | 1800 | 监控间隔（秒） |
| `target_group` | str | - | 推送群号 |
| `target_user` | str | - | 推送用户 QQ |
| `auto_comment` | bool | true | 自动评论开关 |
| `auto_like` | bool | true | 自动点赞开关 |
| `like_probability` | float | 0.8 | 点赞概率（0-1） |
