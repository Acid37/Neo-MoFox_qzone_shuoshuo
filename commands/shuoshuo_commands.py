...
class ReadFeedCommand(BaseCommand):
    ...
    def _parse_options(self, message_text: str) -> tuple[bool, bool, bool, int, float | None, float | None]:
        """解析 /read_feed 命令参数。
        ...
        for token in tokens:
            ...
            if token.startswith("--comment-prob="):
                ...
                continue
            if token in {"--read-only", "--no-like", "--no-comment"}:
                continue
        ...
        return read_only, no_like, no_comment, count, like_prob_override, comment_prob_override
    ...
    async def _force_like_once_if_needed(self, service: BaseService, feed_list: list[dict]) -> tuple[bool, str | None]:
    ...
    async def _force_comment_once_if_needed(self, service: BaseService, feed_list: list[dict]) -> tuple[bool, str | None]:
    ...
    async def execute(self, message_text: str) -> tuple[bool, str]:
        try:
            read_only, no_like, no_comment, count, like_prob_override, comment_prob_override = self._parse_options(message_text)
        except ValueError:
            return False, (
                "参数格式错误，格式: /read_feed [数量] [--read-only] [--no-like] [--no-comment] "
        ...
        try:
            ...
            if (not read_only) and require_like and like_attempt_count == 0:
                forced_like_ok, forced_like_reason = await self._force_like_once_if_needed(service, feed_list)
                like_attempt_count += 1
                if forced_like_ok:
                    liked_count += 1
                elif forced_like_reason:
                    like_failure_reasons[forced_like_reason] += 1

            if (not read_only) and require_comment and comment_attempt_count == 0:
                forced_comment_ok, forced_comment_reason = await self._force_comment_once_if_needed(service, feed_list)
                comment_attempt_count += 1
                if forced_comment_ok:
                    commented_count += 1
                elif forced_comment_reason:
            ...
            if read_only:
                ...
                lines.append(
                    ...
                    f"no_like={no_like}, no_comment={no_comment}"
                )
